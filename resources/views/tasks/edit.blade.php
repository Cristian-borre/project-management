                ...
                <form method="POST" action="{{ route('tasks.update', $task) }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="title" name="title" value="{{ $task->title }}"
                            {{ $task->created_by != auth()->id() ? 'disabled' : '' }} required>
                    </div>

                    <div class="mb-3">
                        <label for="description" class="form-label fw-semibold">Descripción</label>
                        <textarea class="form-control" id="description" name="description" rows="4" {{ $task->created_by != auth()->id() ? 'disabled' : '' }}>{{ $task->description }}</textarea>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" class="form-control" id="due_date" name="due_date"
                                value="{{ $task->due_date }}" {{ $task->created_by != auth()->id() ? 'disabled' : '' }}>
                        </div>

                        <div class="col-md-6 mb-3">
                            <label for="priority" class="form-label">Prioridad</label>
                            <select class="form-control" id="priority" name="priority" {{ $task->created_by != auth()->id() ? 'disabled' : '' }}>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-4">
                            ...
                            <select class="form-control" id="assigned_to" name="assigned_to" {{ $task->created_by != auth()->id() ? 'disabled' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control" id="project_id" name="project_id" {{ $task->created_by != auth()->id() ? 'disabled' : '' }}>
